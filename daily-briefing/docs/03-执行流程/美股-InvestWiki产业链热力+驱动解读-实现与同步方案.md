...
**2026-09-09 及更早**的热力数据，其「市值加权」列失真。

原因：`fetch_industry_chain_heat.py` 曾把 yfinance 返回的**本地币种市值直接当作 USD** 使用（韩元约放大 1380 倍、日元约 150 倍），导致亚洲标的在市值加权口径中权重畸高，该列数值不可引用。**已于美东 2026-09-11 的数据起修复。**

影响范围（逐文件、不扩散——每日热力都是从当天行情重新计算的，不继承前一日产物）：

| 产物 | 市值加权列 | 等权列 |
|---|---|---|
| `industry_chain_heat_2026-09-09.json`、`2026-09-10.json` | ❌ 失真 | ✅ 正常 |
| `industry_chain_heat_2026-09-11.json` 起 | ✅ 正常 | ✅ 正常 |
| 由上述失真数据渲染的 `2026-09-10`、`2026-09-11` 两期产业链报告 | ❌ 失真 | ✅ 正常 |

**处理方式**：报告结论一律以**等权**为准（等权本就是主排序口径），上述历史报告的排序、结构解读与结论均不受影响，仅「市值加权」这一参考列不可引用。按 8.4 的原则，这些历史报告**不回算**。

> 注：早期版本另有重名段数口径差异（原始 81 段 vs canonical 65 段），引用历史报告时需一并注意。
